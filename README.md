...
| start_crawl_and_scan | Launch a crawl and audit scan against a URL |
...
- Host allowlist: tools only operate on hosts you have explicitly approved
...
This extension complements the official Burp MCP proxy, not replaces it. Run both simultaneously:

- Companion on port 9877: scope-aware scanner, sitemap, annotations, cookies
- Official proxy on port 9876: raw proxy history, Repeater, Intruder, Collaborator, encoding tools
...
├── server/
│   ├── McpServerManager.kt    # SSE server and MCP protocol handling
